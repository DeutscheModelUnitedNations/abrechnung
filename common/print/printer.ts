...
export class PDFDrawer<idType extends _id> {
  ...
  async attachReceipts(receiptMap: ReceiptMap<idType>) {
    const failedToRenderWarning = async (receipt: ReceiptMapEntry<idType>) => {
      const fileName = `${receipt._id}${receipt.type === 'application/pdf' ? '.pdf' : ''}`
      ...
      try {
        ...
        if (receipt.type === 'application/pdf') {
          try {
            ...
            firstPageOfReceipt = await failedToRenderWarning(receipt)
          }
        } else {
          let image: PDFImage
