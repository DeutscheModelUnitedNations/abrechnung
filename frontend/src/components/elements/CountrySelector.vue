...
if (!props.modelValue && APP_DATA.value) {
  const allCountries = APP_DATA.value.user.settings.lastCountries.concat(APP_DATA.value.countries)
  const deCountry = allCountries.find((c: CountrySimple) => c._id === 'DE')
  if (deCountry) {
    emits('update:modelValue', deCountry)
  }
}
</script>
